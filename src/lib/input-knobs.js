...
window.addEventListener('load', () => {
  const op = window.inputKnobsOptions || {};
...
<use xlink:href="#K" transform="rotate(-135,32,32)"/>`;
  ...
  const makeHSliderFrames = (fr, fg, bg, w, h) => {
...
<g id="K"><circle x="${w / 2}" y="0" r="${h / 2 * 0.9}" fill="${fg}"/></g></defs>`;
  ...
  const makeVSliderFrames = (fr, fg, bg, w, h) => {
    console.log(fr, fg, bg, w, h);
    let r = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${w}" height="${fr * h}" viewBox="0 0 ${w} ${fr * h}" preserveAspectRatio="none">
...
<circle id="K" x="0" y="0" r="${w / 2 * 0.9}" fill="${fg}"/></defs>`;
  ...
  const initSwitches = (el) => {
    let w; let h; let d; let fg; let bg;
    ...
    el.refresh = () => {
      ...
      if (src) el.style.backgroundImage = `url(${src})`;
        ...
        const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h * 2}" viewBox="0 0 ${w} ${h * 2}" preserveAspectRatio="none">
  ...
  const initKnobs = (el) => {
    ...
    el.refresh = () => {
      ...
      if (src) {
        ...
        // console.log(svg);
        ik.sprites = 100;
        el.style.backgroundImage = `url(data:image/svg+xml;base64,${window.btoa(svg)})`;
    ...
    el.setValue = (v) => {
      v = (Math.round((v - ik.valrange.min) / ik.valrange.step))
        * ik.valrange.step + ik.valrange.min;
    ...
    ik.pointerdown = (ev) => {
      ...
      const cx = (rc.left + rc.right) * 0.5; const cy = (rc.top + rc.bottom) * 0.5;
      const dx = ev.clientX; const dy = ev.clientY;
      ...
      if (ik.itype !== 'k' && op.sliderMode === 'abs') {
        dv = (ik.valrange.min + ik.valrange.max) * 0.5
          + ((dx - cx) / ik.sensex - (dy - cy) / ik.sensey)
          * (ik.valrange.max - ik.valrange.min);
        el.setValue(dv);
      }
      ik.dragfrom = {
        x: ev.clientX, y: ev.clientY, a: Math.atan2(ev.clientX - cx, cy - ev.clientY), v: +el.value
      };
    ...
    ik.pointermove = (ev) => {
      ...
      const cx = (rc.left + rc.right) * 0.5; const cy = (rc.top + rc.bottom) * 0.5;
      if (ev.touches) ev = ev.touches[0];
      const dx = ev.clientX - ik.dragfrom.x; const dy = ev.clientY - ik.dragfrom.y;
      ...
      switch (ik.itype) {
        case 'k':
          switch (op.knobMode) {
            ...
            case 'circularabs':
              if (!ev.shiftKey) {
                dv = ik.valrange.min + (da / Math.PI * 0.75 + 0.5)
                  * (ik.valrange.max - ik.valrange.min);
                el.setValue(dv);
              }
              break;
    ...
    el.redraw = (f) => {
      if (f || ik.valueold !== el.value) {
        const v = (el.value - ik.valrange.min) / (ik.valrange.max - ik.valrange.min);
